	...
	<div class="wrapper">
		<div class="sidebar" data-background-color="white"
			...
			<div class="sidebar-wrapper">
				<div class="logo">
					<a href="#" class="simple-text"> 健身俱乐部管理系统 </a>
				</div>
			...
			<nav class="navbar navbar-default">
				<div class="container-fluid">
					<div class="navbar-header">
						...
						<a class="navbar-brand" href="#">会员列表</a>
					</div>
										...
										<tbody>
											<c:forEach var="list" items="${findUserList}">
												<tr>
													...
													<td>${list.name}</td>
													<td>${list.pwd}</td>
													<td><c:if test="${list.sex==0}">女</c:if> <c:if
															test="${list.sex==1}">男</c:if></td>
													<td>${list.age}</td>
													<td>${list.tel}</td>
													<td>${list.address}</td>
													...
													<td>
														...
														<button class="btn btn-primary btn-fill "
															onclick="update(${list.id},'${list.account}','${list.name}','${list.pwd}',${list.sex},${list.age},'${list.tel}','${list.address}')">编辑</button>
														<button class="btn btn-danger btn-fill "
															onclick="del(${list.id})">删除</button>
													</td>
												</tr>
											</c:forEach>
										</tbody>
